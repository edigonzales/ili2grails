    ...
    <div class="ili-form-section-fields">
        <g:each in="\${section.fields ?: []}" var="fieldName">
            ...
            <g:else>
                ...
                <g:set var="fieldHasErrors" value="\${hasErrors(bean: this.${propertyName}, field: fieldName)}" />
                <div id="field-\${fieldName}"
                     class="ili-form-field \${hasErrors(bean: this.${propertyName}, field: fieldName, 'has-error')}"
                     data-form-field="\${fieldName}">
                    <f:field bean="${propertyName}"
                             property="\${fieldName}"
                             label="\${message(code: '${propertyName}.' + fieldName + '.label', default: fieldInfo.label ?: fieldName)}"
                    ...
                    <g:hasErrors bean="\${this.${propertyName}}" field="\${fieldName}">
                        <div id="field-\${fieldName}-error" class="invalid-feedback d-block" data-field-error="\${fieldName}">
                            <g:eachError bean="\${this.${propertyName}}" field="\${fieldName}" var="error">
