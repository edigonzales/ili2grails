            ...
            <g:if test="${hiddenRelationshipFields?.contains(relationshipField)}">
                <div id="field-${relationshipField}" class="ili-field-row mb-3" data-form-field="${relationshipField}">
                    <label class="form-label">
                        ${message(code: 'employee.' + relationshipField + '.label', default: fieldMeta?.get(relationshipField)?.label ?: relationshipField)}
                    </label>
                ...
                <div id="field-${relationshipField}" class="ili-field-row mb-3 ${relationshipRequired?.get(relationshipField) ? 'required' : ''}" data-form-field="${relationshipField}">
                    ...
                    <div class="ili-relationship-picker js-relationship-picker">
                        <input type="search"
                               ...
                               aria-expanded="false"
                               aria-label="${message(code: 'employee.' + relationshipField + '.label', default: fieldMeta?.get(relationshipField)?.label ?: relationshipField)}"
                        ...
                        <g:select name="${relationshipField}.id"
                                  ...
                                  aria-invalid="${hasErrors(bean: this.employee, field: relationshipField) ? 'true' : 'false'}"
                                  class="form-select ${hasErrors(bean: this.employee, field: relationshipField, 'is-invalid')}" />
                    </div>
                    ...
                    <g:hasErrors bean="${this.employee}" field="${relationshipField}">
                        <div id="relationship-${relationshipField}-error" class="invalid-feedback d-block">
                            <g:eachError bean="${this.employee}" field="${relationshipField}" var="error">
