            ...
            <g:if test="${hiddenRelationshipFields?.contains(relationshipField)}">
                <div id="field-${relationshipField}" class="ili-field-row mb-3" data-form-field="${relationshipField}">
                    <label class="form-label">
                        ${message(code: 'department.' + relationshipField + '.label', default: fieldMeta?.get(relationshipField)?.label ?: relationshipField)}
                    </label>
                ...
                <div id="field-${relationshipField}" class="ili-field-row mb-3 ${relationshipRequired?.get(relationshipField) ? 'required' : ''}" data-form-field="${relationshipField}">
                    ...
                    <div class="ili-relationship-picker js-relationship-picker">
                        <input type="search"
                               ...
                               placeholder="${message(code: 'ili2grails.list.relationshipSearch', default: 'Suchen')}"
                               autocomplete="off"
                               role="combobox"
                               aria-autocomplete="list"
                               aria-haspopup="listbox"
                               aria-expanded="false"
                               aria-label="${message(code: 'department.' + relationshipField + '.label', default: fieldMeta?.get(relationshipField)?.label ?: relationshipField)}"
                        ...
                        <div id="relationship-${relationshipField}-results"
                             ...
                             role="listbox"
                             aria-label="${message(code: 'ili2grails.list.relationshipSelection', default: 'Beziehungsauswahl')}"
                             aria-live="polite"
                             hidden></div>
                        <g:select name="${relationshipField}.id"
                                  ...
                                  optionValue="label"
                                  value="${relationshipValues?.get(relationshipField)}"
                                  noSelection="${relationshipRequired?.get(relationshipField) ? null : ['': message(code: 'ili2grails.js.noSelection', default: 'Keine Auswahl')]}"
                                  ...
                                  aria-invalid="${hasErrors(bean: this.department, field: relationshipField) ? 'true' : 'false'}"
                                  class="form-select ${hasErrors(bean: this.department, field: relationshipField, 'is-invalid')}" />
                    </div>
                    <div id="relationship-${relationshipField}-help" class="form-text"><g:message code="ili2grails.form.relationshipSearch" default="Auswahl über Suche oder Liste."/></div>
                    <g:hasErrors bean="${this.department}" field="${relationshipField}">
                        <div id="relationship-${relationshipField}-error" class="invalid-feedback d-block">
                            <g:eachError bean="${this.department}" field="${relationshipField}" var="error">
